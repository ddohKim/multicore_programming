...
—————————------
Phase3.
프로세스를 background에서도 실행될 수 있도록 & 를 명령어 마지막에 붙여 bg 에서 작동할 수 있도록 하였다.
Jobs 명령어로 어떤 job이 어느 환경에 있고 또 현재 상태는 어떤 상태인지, job 번호는 몇번인지 등을 볼 수 있도록 했다.
fg %# , bg %#, kill %# 명령어로 해당하는 번호에 대해 상태변화를 시킬 수도 있다.
ctrl+c, ctrl+z  키로 stdint, stdstp signal을 발생시켜 프로세스를 중간에 interrupt 하거나 suspend 할 수 있다. 
